...
export const GateHandler = {
    unwrapCredentials: async function (exchangeId) {
    ...
    futuresOrder: async function (c) {
        try {
            ...
            // update leverage
            const resLeverage = await GateServices.updateLeverage(body.contract, body.leverage);
            console.log(resLeverage, "resLeverage");
            // update margin mode
            const resMarginMode = await GateServices.updateMarginMode(body.contract, body.leverage_type);
            ...
            await redis.hset(`gate:creds:${body.user_id}`, "apiKey", api_key, "apiSecret", api_secret);
    ...
    //   }
    // },
    futuresOrderDb: async function (c) {
        try {
            const body = (await c.req.json());
            const { api_key, api_secret, user_id } = await GateHandler.unwrapCredentials(body.exchange_id);
            GateServices.initialize(api_key, api_secret);
            const allReturn = {
                message: "ok",
                data: null,
            ...
            try {
                const tradeStatus = () => {
                    if (resPlaceOrder?.status === "finished" &&
                ...
                const addData = {
                    ...
                    take_profit_executed: allReturn.data?.take_profit?.status === "open" ? true : false,
                    ...
                    stop_loss_executed: allReturn.data?.stop_loss?.status === "open" ? true : false,
    ...
    closePositionDb: async function (c) {
        try {
            // GateServices.initialize(process.env.GATE_API_KEY!, process.env.GATE_API_SECRET!);
            const body = (await c.req.json());
            const { api_key, api_secret, user_id } = await GateHandler.unwrapCredentials(body.exchange_id);
            ...
            const running_trades = await postgresDb.query.trades.findMany({
                where: and(eq(trades.exchange_id, body.exchange_id), eq(trades.contract, body.contract), eq(trades.status, 'waiting_targets'))
            });
            const closed_trades = await Promise.all(running_trades.map(async (trade) => {
                const orderPayload = {
                    contract: body.contract,
