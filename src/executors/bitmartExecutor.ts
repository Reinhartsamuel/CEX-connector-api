...
export const BitmartExecutor: ExchangeExecutor = {
  async execute(ctx: ExecutorContext): Promise<ExecutorResult> {
    ...
    // BitMart uses api_passphrase as memo/uid
    BitmartServices.initialize(ctx.api_key, ctx.api_secret, api_passphrase!);
...
async function openPosition(ctx: ExecutorContext): Promise<ExecutorResult> {
  ...
  // Set leverage (BitMart does NOT support margin mode setting)
  await BitmartServices.updateLeverage(contract, leverage);

  const res = await BitmartServices.placeOrder({
    contract,
    position_type,
  ...
  const tradeStatus = BitmartServices.mapCcxtStatusToDb(ccxtStatus, order_type);
...
/**
 * Compute order quantity (base asset amount) for BitMart Futures.
 * Formula: floor((capital * leverage) / price)
 * BitMart uses base asset quantity (e.g. BTC), not contracts — no multiplier needed.
 */
function computeSize(autotrader: Autotrader, overrides: SignalOverrides): number {
  ...
  if (!price || price <= 0) {
    throw new Error(
      `[BitmartExecutor] price is required for sizing. ` +
