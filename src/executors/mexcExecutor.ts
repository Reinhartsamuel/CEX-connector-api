...
export const MexcExecutor: ExchangeExecutor = {
  async execute(ctx: ExecutorContext): Promise<ExecutorResult> {
    const { action } = ctx;

    MexcServices.initialize(ctx.api_key, ctx.api_secret);
...
async function openPosition(ctx: ExecutorContext): Promise<ExecutorResult> {
  ...
  await Promise.all([
    MexcServices.updateLeverage(contract, leverage),
    MexcServices.updateMarginMode(contract, leverage_type),
  ]);

  const res = await MexcServices.placeOrder({
    contract,
    position_type,
  ...
  if (!res?.id) {
    ...
    console.error('[MexcExecutor] placeOrder failed:', errMsg);
    return { success: false, error: `MEXC order rejected: ${errMsg}` };
  }

  const ccxtStatus: string = res.status ?? '';
  const tradeStatus = MexcServices.mapCcxtStatusToDb(ccxtStatus, order_type);
  ...
  try {
    ...
    console.error('[MexcExecutor] Failed to persist trade to DB:', err);
  }
  ...
  const orderResults = await Promise.all(
    running_trades.map(async (trade) => {
      ...
      try {
        ...
        console.error('[MexcExecutor] closePosition failed:', err.message);
...
/**
 * Compute order quantity (base asset amount) for MEXC Futures.
 * Formula: floor((capital * leverage) / price)
 * MEXC uses base asset quantity (e.g. BTC), not contracts — no multiplier needed.
 */
function computeSize(autotrader: Autotrader, overrides: SignalOverrides): number {
  ...
  if (!price || price <= 0) {
    throw new Error(
      `[MexcExecutor] price is required for sizing. ` +
