...
// ------------------------------------------- //
async function ensureConnection(userId, contractList = ["!all"]) {
    ...
    ws.on("message", (raw) => onWsMessage(userId, raw));
    ws.on("close", (code, reason) => onWsClose(userId, code, reason));
    ws.on("error", (err) => console.error(`WS Worker: WS error (${userId})`, err));
}
// ------------------------------------------- //
//             EVENT HANDLERS
// ------------------------------------------- //
function onWsOpen(userId, ws, creds) {
    ...
    }, 25000); // must be ≤ 30s for Gate
}
async function onWsMessage(userId, raw) {
    let msg;
    try {
            ...
            if (item.id_string) {
                switch (event) {
                    case "order_filled_open":
                        ...
                        console.log("trade Data::::: please handle TP/SL", JSON.stringify(tradeData, null, 2));
                        break;
    ...
    if (channel === "futures.positions") {
        console.log(`📥 POSITIONS UPDATE (${userId}):`, JSON.stringify(msg, null, 2));
...
function onWsClose(userId, code, reason) {
    console.warn(`WS Worker: WS CLOSED user=${userId} code=${code} reason=${reason.toString()}`);
    const state = connections.get(userId);
    if (!state)
        return;
    if (state.pingInterval)
...
// --------------------- classifier helpers ---------------------
function classifyOrderEvent(item) {
    // subscribe ack
    ...
    if (left > 0 && left < Math.abs(size || 0))
        return "order_partial_fill";
    return "order_other";
}
function classifyPositionEvent(item, prev) {
    const newSize = Number(item?.size ?? 0);
    const prevSize = prev ? Number(prev.size ?? 0) : 0;
    if (prev === undefined && newSize !== 0)
        return "position_opened";
    if (prev !== undefined && newSize === 0 && prevSize !== 0)
        return "position_closed";
    if (prev !== undefined && newSize !== prevSize)
        return "position_changed";
    return "position_heartbeat";
}
async function handlePositionItem(userId, item) {
